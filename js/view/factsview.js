define(function(require){
    ...
    var $ = require('jquery'),
        ...
        template = require('text!../../html/template/facts.html');

    var FactsView = TemplateView.extend({
        ...
        init: function() {
    		...
    		this.facts.refresh.add(function() {
    			var facts_data = this.facts.data.facts;
    			...
    			if (this.facts.each_scene_on_new_page()) {				
    			...
    			var primary_characters = this.facts.get_characters_by_level(1).map(function(ch){return ch.name}).join(', ');
    			$('#facts-primary-characters').html(primary_characters || '-');
    			var secondary_characters = this.facts.get_characters_by_level(2).map(function(ch){return ch.name}).join(', ');
